...
def get_file_content_unsafe(working_directory, file_path):
  ...
  file_path_abs_path = path.abspath(path.join(working_directory, file_path))
  
  if not file_path_abs_path.startswith(working_dir_abs_path):
    return f'Error: Cannot read "{file_path}" as it is outside the permitted working directory'
  
  if not path.isfile(file_path_abs_path):
